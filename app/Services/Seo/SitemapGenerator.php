    ...
    private function getRoutePairs(): array
{
    ...
    $hasUpdatedAt = Schema::hasColumn($toursTable, 'updated_at');
    ...
    if ($hasUpdatedAt) {
        // Берём одну дату на пару маршрута
        $q->select(array_merge($baseSelect, [DB::raw('MAX(t.updated_at) as updated_at')]))
          ->groupBy($baseSelect);
    } else {
        // Колонки нет — подставляем NULL, чтобы map() не ругался
        $q->select(array_merge($baseSelect, [DB::raw('NULL as updated_at')]))
          ->distinct();
    }
