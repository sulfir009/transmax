...
$('.filter_city_select').select2({
 ...
 $('.order_bus_select').select2({
   ...
   function toggleFilterCalendar() {
       const filterDateInput = document.querySelector('.filter_date');
       if (filterDateInput && filterDateInput._flatpickr) {
           filterDateInput._flatpickr.open();
       }
   }
            ...
            success: function(response) {
                ...
                if (highlightedDaysString) {
                    ...
                    filterDatePicker = flatpickr(filterInput, {
                        ...
                        altInput: true,
                        altFormat: "d.m.Y",
                        defaultDate: window.filterDate || 'today',
                        locale: '<?php echo $Router->lang?>',
                        static: true,
                        disableMobile: true,
                        appendTo: filterInput.parentElement,
...
<?php
$departure = $_SESSION['order']['fromCityId'] ?? '';
$arrival = $_SESSION['order']['toCityId'] ?? '';
$orderDate = $_SESSION['order']['date'] ?? '';
?>
document.addEventListener("DOMContentLoaded", function() {
