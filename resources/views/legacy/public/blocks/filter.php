...
if (isset($_POST['departure']) && (int)$_POST['departure'] > 0){
    $filterDeparture = (int)$_POST['departure'];
    $_SESSION['filter']['departure'] = $filterDeparture;
}elseif (empty($_POST['departure']) && !empty($_SESSION['filter']['departure'])){
    $filterDeparture =  $_SESSION['filter']['departure'];
}

if (isset($_POST['arrival']) && (int)$_POST['arrival'] > 0){
    $_SESSION['filter']['arrival'] =  (int) $filterArrival = (int) $_POST['arrival'];
}elseif (empty($_POST['arrival']) && !empty($_SESSION['filter']['arrival'])){
...
if (isset($_POST['date']) && trim($_POST['date']) != ''){
    $filterDate = implode('-',array_map('intval', explode('-', $_POST['date'])));
    $_SESSION['filter']['date'] = $filterDate;
}elseif (empty($_POST['date']) && !empty($_SESSION['filter']['date'])){
    $filterDate = $_SESSION['filter']['date'];
}

// Преобразуем 'today' в текущую дату в формате Y-m-d
if ($filterDate == 'today') {
...
if (isset($_POST['adults'])) {
    $_SESSION['filter']['adults'] = $adults = (int) $_POST['adults'];
}elseif (empty($_POST['adults']) && !empty($_SESSION['filter']['adults'])){
    $adults = $_SESSION['filter']['adults'];
}

if (isset($_POST['kids'])) {
    $_SESSION['filter']['kids'] = (int) $kids = (int) $_POST['kids'];
}elseif (empty($_POST['kids']) && !empty($_SESSION['filter']['kids'])){
