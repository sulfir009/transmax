...
if ($cleanPost['request'] === 'more_buses') {
    ...
    foreach ($getBuses as $k => $bus) {
        ...
        <div class="bus flex-row gap-30">
            <div class="col-lg-6">
                <div class="bus_img">
                    <img src="<?php echo  asset('images/legacy/upload/buses/' . $bus['image']); ?>" alt="bus" class="fit_img">
                </div>
            </div>
            <div class="col-lg-6">
                <div class="bus_info">
                    ...
                    <div class="bus_seats flex_ac h4_title">
                        <?php echo  __('dictionary.MSG_MSG_BUSES_KILIKISTI_MISCI') ?>
                        <span class="total_seats h2_title">
                                        <?php echo  $bus['seats_qty'] ?>
                                    </span>
                    </div>
                    <div class="bus_info_delimiter"></div>
                    <div class="bus_options">
                        <div class="flex-row gap-30">
                            <?php $getBusAdditionalOptions = $Db->getAll("SELECT title_" . $Router->getLang() . " AS title FROM `" . DB_PREFIX . "_buses_options` WHERE active = '1'
                            ...
                            foreach ($getBusAdditionalOptions as $key => $additionalOption) { ?>
                                <div class="col-sm-4 col-xs-6">
                                    <div class="bus_option flex_ac par">
                                        ...
                                        <?php echo  $additionalOption['title'] ?>
                                    </div>
                                </div>
                            <?php } ?>
                        </div>
                    ...
                    <button class="order_bus_link h4_title flex_ac blue_btn"
                            onclick="toggleOrderBus('<?php echo  $bus['id'] ?>')">
                        <?php echo  __('dictionary.MSG_MSG_BUSES_ZAMOVITI_AVTOBUS') ?>
                    </button>
                </div>
            </div>
        </div>
...
if ($cleanPost['request'] === 'check_OrderTicket') {
    ...
    $departureTime = $_SESSION['order']['departure_time'];
    ...
    if (strtotime($date) == strtotime(date('Y-m-d',time()))){
        ...
        if (date('Y-m-d') === date('Y-m-d',$toursDepartureClosed)) {
            if (strtotime($checkTicketDeparture['departure_time']) - $toursDepartureClosed - strtotime(date('H:i:s',time())) > 0){
                $canOrderTicket = false;
            }
        } else {
            if (strtotime($checkTicketDeparture['departure_time']) - $toursStopsClosed - strtotime(date('H:i:s',time())) > 0){
                $canOrderTicket = false;
            }
        }
    }
...
if ($cleanPost['request'] === 'route_details') {
    ...
    <div class="route_details_popup_content_wrapper">
        ...
        <div class="route_details_popup_content">
            <div class="route_details_title route_details_block_title h3_title flex_ac" onclick="toggleInfoBlock(this)">
                <?php echo  __('dictionary.MSG_MSG_TICKETS_MARSHRUT') ?>
                <img src="<?php echo  asset('images/legacy/common/arrow_down.svg'); ?>" alt="arrow left">
            </div>
            <div class="route_details_points">


                <?php $getStops = $Db->getAll("SELECT stop_id,arrival_time FROM `" . DB_PREFIX . "_tours_stops`
    ...
    WHERE tt.tour_id = '".(int)$cleanPost['id']."' ");
                ...
                foreach ($getStops as $k => $stop) {
                    ...
                    ?>
                    <div class="route_details_point flex_ac <?php if ($k == 0){echo 'active';}?>">
                        <div class="route_details_point_time par">
                            <?php echo  date('H:i', strtotime($stop['arrival_time'])) ?>
                        </div>
                        <div class="route_details_point_title par">
                            <?php echo  $stopCity['title'] . ' ' . $stopTitle['title'] ?><?php if ($isTransfer) { ?>
                                <img class="transfer_icon" src="<?php echo  asset('images/legacy/transfer_white.svg'); ?>" alt="Пересадка">
                                <?php echo  __('dictionary.MSG_MSG_SCHEDULE_TRANSFER') . ''; ?>
                            <?php }?>
                            <?php if (!$stopPrice || $stopPrice['price'] == 0){?>
                                <?php echo  __('dictionary.MSG_MSG_TICKETS_NET_POSADKI_/_VYSADKI_PASSAZHIROV')?>
                            <?php }?>
                        </div>
                    </div>
                    <?php
            ...
            <div class="route_details_bus_services_wrapper">
                <div class="route_details_bus_services_title route_details_block_title h3_title flex_ac"
                    ...
                    <?php echo  __('dictionary.MSG_MSG_TICKETS_POSLUGI_V_AVTOBUSI'); ?>
                    <img src="<?php echo  asset('images/legacy/common/arrow_down.svg'); ?>" alt="arrow left">
                </div>
                <?php $getBusOptions = $Db->getAll("SELECT option_id FROM `" . DB_PREFIX . "_buses_options_connector` WHERE bus_id = (SELECT bus FROM `" . DB_PREFIX . "_tours` WHERE id = '" . $cleanPost['id'] . "') ");
                $busOptionsArrays = array_chunk($getBusOptions, 3); ?>
                <div class="route_details_bus_services flex-row gap-24">
                    <?php foreach ($busOptionsArrays as $k => $busOptions) {
                        ?>
                        <div class="col-sm-4 col-xs-12">
                            <?php foreach ($busOptions as $k => $busOption) {
            ...
            <div class="route_details_rules">
                <div class="route_details_rules_title route_details_block_title h3_title flex_ac"
                     onclick="toggleInfoBlock(this)">
                    <?php echo  __('dictionary.MSG_MSG_TICKETS_PRAVILA_POVERNENNYA') ?>
                    <img src="<?php echo  asset('images/legacy/common/arrow_down.svg'); ?>" alt="arrow left">
                </div>
                <div class="route_details_rules_txt par">
                    <?php $returnTxt = $Db->getOne("SELECT text_" . $Router->getLang() . " AS text FROM `" . DB_PREFIX . "_txt_blocks` WHERE id = '7' ") ?>
                    <?php echo  $returnTxt['text'] ?>
                </div>
            </div>
            <div class="route_details_totals flex_ac">
                <div class="route_details_route_price h2_title">
                    <?php echo  $ticketPrice['price'] . ' ' . __('dictionary.MSG_MSG_TICKETS_GRN'); ?>
                </div>
                <button class="blue_btn flex_ac route_details_buy_btn h4_title"
                        onclick="buyTicket(this,'<?php echo  $cleanPost['id'] ?>','<?php echo (int)$cleanPost['departure']?>','<?php echo (int)$cleanPost['arrival']?>')">
                    <?php echo  __('dictionary.MSG_MSG_TICKETS_KUPITI_KVITOK') ?>
                </button>
            </div>
        </div>
    </div>
            ...
            <div class="route_details_points">
                ...
                foreach ($getStops as $k => $stop) {
                    ...
                    <div class="route_details_point flex_ac <?php if ($k == 0){echo 'active';}?>">
                        ...
                        <div class="route_details_point_title par">
                            <?php echo  $stopCity['title'] . ' ' . $stopTitle['title'] ?>
                        </div>
                    </div>
                    <?php
                } ?>
    ...
    <div class="route_details_popup_content_wrapper">
        ...
        <div class="route_details_popup_content">
            ...
            <div class="route_details_points">
                ...
                $ticketPrice = $Db->getOne("SELECT price FROM `" .  DB_PREFIX . "_tours_stops_prices`  WHERE from_stop = '".(int)$cleanPost['departure']."' AND to_stop = '".(int)$cleanPost['arrival']."' AND tour_id = '".(int)$cleanPost['id']."' ");
                $getStops = $Db->getAll("SELECT stop_id,arrival_time FROM `" . DB_PREFIX . "_tours_stops` WHERE `tour_id` = '" . $cleanPost['id'] . "' ORDER BY stop_num ASC "); ?>
                <?php foreach ($getStops as $k => $stop) {
                    $stopTitle = $Db->getOne("SELECT section_id,title_" . $Router->getLang() . " AS title FROM `" . DB_PREFIX . "_cities` WHERE id = '" . $stop['stop_id'] . "' ");
                    $stopCity = $Db->getOne("SELECT title_" . $Router->getLang() . " AS title FROM `" . DB_PREFIX . "_cities` WHERE id = '" . $stopTitle['section_id'] . "' ") ?>
...
if ($cleanPost['request'] === 'filter') {
    ...
    if (is_array($getTickets) && count($getTickets) > 0) {?>
        <div class="catalog_elements_title h3_title">
            <?php echo  __('dictionary.MSG_MSG_TICKETS_ZNAJDENO') . ' ' . count($getTickets) . ' ' . __('dictionary.MSG_MSG_TICKETS_AVTOBUSIV') ?></div>
        <div class="catalog_elements_subtitle par"><?php echo  __('dictionary.MSG_MSG_TICKETS_CHAS_VIDPRAVLENNYA_TA_PRIBUTTYA_MISCEVIJ') ?></div>
                    ...
                    <div class="flex-row">
                            ...
                            <div class="ticket_info">
                                <div class="ticket_info_header flex_ac">
                                    <div class="ticket_info_date_block flex_ac">
                                        <img src="<?php echo  asset('images/legacy/common/ticket_calendar.svg'); ?>"
                                             alt="calendar">
                                ...
                                <div class="ticket_ride_info_block flex-row gap-30">
                                    <div class="col-lg-4 col-sm-6 col-xs-12">
                                        <div class="ticket_ride_departure ticket_ride_info">
                                            <div class="ticket_ride_time flex_ac">
                                                ...
                                                <span class="btn_txt"><?php echo  date("H:i", strtotime($departureDetails['departure_time'])) ?></span>
                                            </div>
                                            <div class="ticket_ride_city btn_txt">
                                                <?php echo  $departureDetails['city'] ?>
                                            </div>
                                            <div class="ticket_ride_checkpoint manrope">
                                                <?php echo  $departureDetails['station'] ?>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-4 col-sm-6 col-xs-12">
                                        <div class="ticket_ride_arrival ticket_ride_info">
                                            <div class="ticket_ride_time flex_ac">
                                                <img src="<?php echo  asset('images/legacy/common/clock.svg'); ?>"
                                                     alt="clock">
                                                <span class="btn_txt"><?php echo  date('H:i', strtotime($arrivalDetails['arrival_time'])) ?></span>
                                            </div>
                                            <div class="ticket_ride_city btn_txt">
                                                <?php echo  $arrivalDetails['city'] ?>
                                            </div>
                                            <div class="ticket_ride_checkpoint">
                                                <?php echo  $arrivalDetails['station'] ?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-3 hidden-md hidden-sm hidden-xs">
                            <div class="ticket_totals">
                                <div class="ticket_price"><?php echo  $ticketPrice['price'] ?> ₴</div>
                                <button class="ticket_buy_btn flex_ac h5_title blue_btn" onclick="buyTicket(this,'<?php echo  $ticket['id'] ?>','<?php echo $departureDetails['id']?>','<?php echo $arrivalDetails['id']?>')">
    ...
    <?php } else{?>
        <div class="catalog_elements">
            <div class="catalog_elements_title h3_title">
                <?php echo $GLOBALS['dictionary']['MSG_MSG_TICKETS_PO_VYBRANNYM_PARAMETRAM_FILITRA_BILETOV_NE_NAJDENO']?>
            </div>
        </div>
    <?php }
...
if ($cleanPost['request'] === 'order_route'){
    $tourId = (int)$_SESSION['order']['tour_id'];
    $from = (int)$_SESSION['order']['from'];
    $to = (int)$_SESSION['order']['to'];
    $tourDate = $_SESSION['order']['date'];
    $clientName = $_SESSION['order']['name'];
    $clientSurname = $_SESSION['order']['family_name'];
    $clientMail = $_SESSION['order']['email'];
    $clientPhone = $_SESSION['order']['phone'];
    $paymethod = $cleanPost['paymethod'];
    $passengers = (int)$_SESSION['order']['passengers'];
    $uniqId = $_SESSION['order']['order_id'];
            ...
            <div class="return_ticket_route_info">
                <div class="return_ticket_route_info_header flex_ac">
                    <div class="return_ticket_date par flex_ac">
                        <img src="<?php echo  asset('images/legacy/common/calendar_white.svg'); ?>" alt="calendar">
                        <?php
                        $departureTourDate = date('d',strtotime($getTicketInfo['tour_date'])).' '.$month['title'].' '.date('Y',strtotime($getTicketInfo['tour_date']))?>
                        <span> <?php echo $departureTourDate?> </span>
                    </div>
                    <div class="return_ticket_route flex_ac par">
                        <span> <?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_REJS'] ?> </span>
                        <span> <?php echo $getTicketInfo['departure_city'].' - '.$getTicketInfo['arrival_city']?></span>
                    </div>
                    <div class="return_ticket_route no_margin flex_ac par">
                        <span> <?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_AVTOBUS'] ?> </span>
                        <span> <?php echo $getTicketInfo['bus_title']?> </span>
                    </div>
                </div>
                    ...
                    <div class="flex-row gap-30">
                        <div class="col-md-4 col-sm-6">
                            <div class="return_ticket_time flex_ac">
                                <img src="<?php echo  asset('images/legacy/common/clock_light.svg'); ?>" alt="clock light">
                                <span class="btn_txt"><?php echo date('H:i',strtotime($getTicketInfo['departure_time']))?></span>
                            </div>
                            <div class="return_ticket_point btn_txt">
                                <?php echo $getTicketInfo['departure_city']?>
                            </div>
                            <div class="return_ticket_description manrope">
                                <?php echo $getTicketInfo['departure_station']?>
                            </div>
                        </div>
                        <div class="col-md-4 hidden-sm col-xs-12">
                            <div class="return_ticket_logo">
                                <div class="return_ticket_logo_wrapper">
                                    <img src="<?php echo  asset('images/legacy/common/ticket_logo_light.svg'); ?>" alt="ticket logo"
                                         class="fit_img">
                                </div>
                                <span class="manrope par"><?php echo $getTicketInfo['hours'].' '.$GLOBALS['dictionary']['MSG_MSG_FUTURE_GOD'].' '.$getTicketInfo['minutes'].' '.$GLOBALS['dictionary']['MSG_MSG_FUTURE_HV_V_DOROZI']?></span>
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-6">
                            <div class="return_ticket_time">
                                <img src="<?php echo  asset('images/legacy/common/clock_light.svg'); ?>" alt=" clock light">
                                <span class="btn_txt"><?php echo date('H:i',strtotime($getTicketInfo['arrival_time']))?></span>
                            </div>
                            <div class="return_ticket_point btn_txt">
                                <?php echo $getTicketInfo['arrival_city']?>
                            </div>
                            <div class="return_ticket_description manrope">
                                <?php echo $getTicketInfo['arrival_station']?>
                            </div>
                        </div>
                ...
                <div class="row">
                    <select class="return_ticket_reason flex_ac">
                        <option value="" selected disabled><?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_PRICHINA_POVERNENNYA'] ?></option>
                        <?php $getReturnReasons = $Db->getAll("SELECT id,title_".$Router->getLang()." AS title FROM `" .  DB_PREFIX . "_return_reasons`  WHERE active = 1 ORDER BY sort DESC ");
                        foreach ($getReturnReasons AS $k=>$returnReason){?>
                            <option value="<?php echo $returnReason['id']?>"><?php echo $returnReason['title']?></option>
                        <?php }?>
                    </select>
                </div>
                ...
                <div class="return_ticket_rules">
                    <div class="return_ticket_rules_title h3_title">
                        <?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_PRAVILA_POVERNENNYA'] ?>
                    </div>
                ...
                <div class="return_ticket_method">
                    <?php if ($getTicketInfo['payment_status'] === '2') { ?>
                        <div class="h3_title return_ticket_method_title">
                            <?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_SPOSIB_POVERNENNYA_KOSHTIV'] ?>
                        </div>
                        <div class="return_method_row flex_ac">
                            <label class="c_checkbox_wrapper flex_ac">
                                ...
                                <span class="c_checkbox_title par"><?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_BANKIVSIKA_KARTKA'] ?></span>
                            </label>
                            <div class="return_ticket_price h2_title">

                                <?php $totalPrice = ($getTicketInfo['price'] * $getTicketInfo['passagers']) ; ?>
                    ...
                    <?php } else { ?>
                        <div class="return_method_row flex_ac d_none">
                            <label class="c_checkbox_wrapper flex_ac">
                                ...
                                <span class="c_checkbox white"></span>
                                <span class="c_checkbox_title par"><?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_GOTIVKOYU'] ?></span>
                            </label>
                            <div class="return_ticket_price h3_title">
                                <?php echo $getTicketInfo['price']?> <?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_GRN'] ?>
                            </div>
                        </div>
                    <?php } ?>
                </div>
                ...
                <div class="return_ticket_method">
                    ...
                    <?php foreach ($getOrderedTickets as $k=>$ticket) { ?>
                        <div class="return_method_row flex_ac ">
                            <label class="c_checkbox_wrapper flex_ac">
                                <input type="checkbox" name="ticket" class="c_checkbox_checker return_tickets" hidden=""  value="<?php echo $ticket['id'];?>">
                                <span class="c_checkbox white"></span>
                                <span class="c_checkbox_title par"><?php echo  $ticket['name'] .' '. $ticket['second_name'] ?></span>
                            </label>
                            <div class="return_ticket_price h3_title">
                                <?php echo $getTicketInfo['price']?> <?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_GRN'] ?>
                            </div>
                ...
                <div class="return_ticket_totals flex_ac">
                    <?php if ($getTicketInfo['payment_status'] === '2') { ?>
                        <div class="return_ticket_price h2_title">
                            <?php echo $totalPrice?><?php echo  $GLOBALS['dictionary']['MSG_MSG_FUTURE_GRN'] ?>
                        </div>
                    <?php } ?>
                    <button class="return_ticket_return_btn blue_btn flex_ac" onclick="returnTicket('<?php echo $getTicketInfo['id']?>', <?php echo $getTicketInfo['price'] ?>)">
...
if ($cleanPost['request'] === 'route_details_schedule'){
    ...
    <div class="schedule_details_content">
        <button class="close_details" onclick="toggleRouteDetailsSchedule('0')">
            <img src="<?php echo  asset('images/legacy/common/arrow_left_blue.svg'); ?>" alt="arrow left">
        </button>
        <div class="schedule_details_block_header h3_title">
            <?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_MARSHRUT']?> <?php echo $departureCity.' - '.$arrivalCity?>
        </div>
        <!--div class="schdule_details_route par">

            <?php foreach ($mainInfo AS $k=>$tourStop){
                $isTransfer = false;
                foreach ($transferStop as $transfer) {
        ...
        <div class="schedule_details_block_header h3_title">
            <?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE__MISCE_POSADKI_V_UKRANI']?>
        </div>
        <ul class="landing_places">
            <?php foreach ($mainInfo AS $k=>$ts){
                if ($ts['stop_country_id'] == 13){?>
                    <li class="par"><?php echo $ts['stop_city'].' '.$ts['stop_title']?></li>
                <?php }
            }?>
        </ul-->
        <div class="departure_block_title">
            <span><?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_VIDPRAVKA']?></span>
            <!--span class="hidden-sm hidden-xs"><?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_FINISH']?></span-->
        </div>
        <div class="schedule_route">
            <?php foreach ($mainInfo AS $key=>$tourStop){
                $isTransfer = false;
                foreach ($transferStop as $transfer) {
                    if ($tourStop['stop_id'] == $transfer['transfer_station']) {
                        $isTransfer = true;
                ...
                <div class="schedule_route_row">
                    <div class="schedule_route_point_row">
                        <div class="schedule_route_point">
                            <div class="schedule_route_point_point <?php if ($key == 0){echo 'active';}?> "></div>
                        </div>
                        <div class="schedule_route_point_time"><?php echo date('H:i',strtotime($tourStop['departure_time']))?></div>
                    </div>
                    <a href="<?php echo $tourStop['google_maps_link']?>" class="schedule_route_stop" target="_blank">
                        <div class="schedule_route_city"><?php echo $tourStop['stop_city']?><?php if ($isTransfer) {?>
                                <img class="transfer_icon" src="<?php echo  asset('images/legacy/transfer.svg'); ?>" alt="Пересадка">
                                <?php echo  $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_TRANSFER'] . ''; ?>
                            <?php }?></div>
                        <div class="schedule_route_station"><?php echo $tourStop['stop_title']?></div>

                    </a>
                    <!--div class="schedule_route_point_row hidden-sm hidden-xs">
                        <div class="schedule_route_point_time"><?php echo date('H:i',strtotime($tourStop['arrival_time']))?></div>
                        <div class="schedule_route_point">
                            <div class="schedule_route_point_point <?php if ($key == (count($mainInfo) - 1)){echo 'active';}?>"></div>
                        </div>
                    </div-->
                </div>
            <?php }?>
        ...
        <div class="arrival_block_title">
            <span><?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_FINISH']?></span>
            <!--span class="hidden-sm hidden-xs"><?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_VIDPRAVKA']?></span-->
        </div>

        <div class="schedule_route_important_txt">
            <div class="sr_txt_title h3_title"><?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_VAZHLIVO']?></div>
            <div class="sr_txt">
                <?php $importantTxt = $Db->getOne("SELECT text_".$Router->getLang()." AS text FROM `" .  DB_PREFIX . "_txt_blocks`  WHERE id = 11");
        ...
        <div class="sr_services">
            <div class="sr_services_title_row h3_title" onclick="toggleDetailsServices(this)">
                <div class="sr_services_title h3_title">
                    <?php echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_POSLUGI_V_AVTOBUSI']?>
                </div>
                <img src="<?php echo  asset('images/legacy/common/arrow_down.svg'); ?>" alt="arrow">
            </div>
...
if ($cleanPost['request'] === 'route_price_details'){
    ...
    <div class="schedule_details_content">
        ...
        <div class="route_price_details_table par">
            <?php if (count($currentStopsPrices) > 4) {
            ?>
        ...
        <div class="routes_prices_list">
            ...
            <?php foreach ($currentStopsPrices as $k=>$routeStopPrice) {
                ...
                <div class="route_stops_prices_list">
                    <div class="route_stops_stop_info">
                        <div class="route_stop_title"><?php echo $getFromStopData['0']['city']?> </div>
                        <div class="route_stop_station_title">  <?php echo $getFromStopData['0']['station']?></div>
                    </div>
                    <div class="route_stops_stop_info">
                        <div class="route_stop_title"><?php echo  $getToStopData['0']['city']?></div>
                        <div class="route_stop_station_title"><?php echo $getToStopData['0']['station']?></div>
                    </div>
                    <div class="route_stops_stop_info route_stops_prices_price"><?php echo $routeStopPrice['price']?> ГРН
                        <button class="buy_btn h5_title"
                                onclick="buyTicketFromSchedule(this,'<?php echo  $currentStopsPrices['0']['tour_id'] ?>','<?php echo  $getFromStopData['0']['stop_id'] ?>','<?php echo  $getToStopData['0']['stop_id'] ?>','<?php echo  $nearestDepartureDate ?>')">
                            <?php echo  $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_KUPITI_KVITOK'] ?>
                        </button>
                    </div>
...
if ($cleanPost['request'] === 'order_mail') {
    \Illuminate\Support\Facades\Log::error('laravel order_mail');
    ...
    //$_POST = request()->all();
    $_POST = decodeHtmlEntities($_POST);
