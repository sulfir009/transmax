...
<div class="wrapper">
    ...
    <div class="content">
            ...
            <div class="tabs_links_container hidden-xs">
                <div class="private_links">
                    <div class="private_link_wrapper">
                        <a href="<?php echo $Router->writelink(80)?>" class="private_tab h4_title">
                    ...
                    <div class="private_link_wrapper">
                        <a href="<?php echo $Router->writelink(82)?>" class="private_tab h4_title active">
        ...
        <div class="page_content_wrapper">
            <div class="container">
                <?php $clientCard = $Db->getOne("SELECT * FROM `" .  DB_PREFIX . "_clients_cards`WHERE client_id = '".$User->id."' ");
                if (!$clientCard){?>
                    <div class="private_empty_block">
                        <div class="private_empty_block_title h2_title">
                            <?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_U_VAS_POKI_NE_PIDDNANI_SPOSOBI_OPLATI']?>
                        </div>
                        <div class="private_empty_block_subtitle">
                            <?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_VVEDITI_DANI_KARTKI_SCHOB_VAM_BULO_ZRUCHNISHE_KUPUVATI_BILETI']?>
                        </div>
                    </div>
                <?php }else{?>
                    <div class="flex-row gap-x-30 gap-y-60">
                        <div class="col-xxl-7">
                            <div class="shadow_block private_payment_data_block">
                                <div class="private_payment_data_header h2_title">
                                    <?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_DANI_KARTKI']?>
                                </div>
                                <div class="flex-row gap-30 payment_data_form">
                                    <div class="col-xxl-6">
                                        <?php $clientCard = $Db->getOne("SELECT * FROM `" .  DB_PREFIX . "_clients_cards`WHERE client_id = '".$User->id."' ")?>
                                        <div class="payment_form_fields">
                                            <div class="flex-row gap-30">
                                                <div class="col-md-12">
                                                    <input type="text" class="c_input" placeholder="<?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_XXXX_XXXX_XXXX_XXXX']?>" id="card_number"
                                                        <?php if (trim($clientCard['card_number']) != ''){?>
                                                            value="<?php echo $clientCard['card_number']?>"
                                                        <?php }?>>
                                                </div>
                                                <div class="col-xs-4">
                                                    <input type="text" class="c_input card_valid_date" placeholder="<?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_12_/_25']?>" id="card_valid_date"
                                                        <?php if (trim($clientCard['valid_date']) != ''){?>
                                                            value="<?php echo $clientCard['valid_date']?>"
                                                        <?php }?>
                                                    >
                                                </div>
                                                <div class="col-xs-8">
                                                    <div class="row">
                                                        <input type="password" class="c_input cvv_input" placeholder="***" id="card_cvv"
                                                            <?php if ((int)$clientCard['cvv'] > 0){?>
                                                                value="<?php echo $clientCard['cvv']?>"
                                                            <?php }?>
                                                        >
                                                        <button class="show_cvv_btn" onclick="toggleCvv(this)"></button>
                                                    </div>
                                                </div>
                                                <div class="col-md-12">
                                                    <input type="text" class="c_input" placeholder="<?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_IMYA_VLASNIKA_KARTKI']?>" id="cardholder_name"
                                                        <?php if (trim($clientCard['cardholder_name']) != ''){?>
                                                            value="<?php echo $clientCard['cardholder_name']?>"
                                                        <?php }?>
                                                    >
                                                </div>
                                            </div>
                                        ...
                                        <div class="row">
                                            <div class="payment_form_paymethod_logos flex_ac">
                                                <img src="<?php echo  asset('images/legacy/common/visa.svg'); ?>" alt="visa" class="fit_img">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-xxl-6">
                                        <div class="payment_form_right">
                                            <div class="payment_form_image">
                                                <img src="<?php echo  asset('images/legacy/common/credit_card.png'); ?>" alt="credit card" class="fit_img">
                                            </div>
                                            <button class="blue_btn h4_title edit_private_payment_btn" onclick="saveClientPaymentData()">
                                                <img src="<?php echo  asset('images/legacy/common/pencil.svg'); ?>" alt="pencil">
                                                <?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_ZMINITI_DANI']?>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-xxl-5">
                            <div class="private_payments_clarification_block">
                                <img src="<?php echo  asset('images/legacy/cards.png'); ?>" alt="cards" class="fit_img">
                                <div class="private_payments_clarification_txt h5_title">
                                    <?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_VSYA_PLATIZHNA_INFORMACIYA']?>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function saveClientPaymentData(){
        ...
        $.ajax({
           type:'post',
            ...
            success:function(response){
                removeLoader();
                if ($.trim(response) == 'ok'){
                    out('<?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_DANNYE_USPESHNO_SOHRANENY']?>');
                }else if($.trim(response) == 'err'){
                    out('<?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_NE_UDALOSI_SOHRANITI_DANNYE']?>','<?php echo $GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_POPROBUJTE_POZZHE']?>');
                }
            }
        })
    }
