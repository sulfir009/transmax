...
<head>
    <link rel="stylesheet" href="<?php echo  mix('css/legacy/libs/slick/jquery.mCustomScrollbar.min.css') ?>">
            ...
            <div class="shedule_table_container">
                ...
                <div class="shedule_table_wrapper">
                    <table class="shedule_table">
                        ...
                        <tr>
                            <th>
                                <?php echo  __('dictionary.MSG_MSG_SCHEDULE_KRANA') ?>
                            </th>
                            <th><?php echo  __('dictionary.MSG_MSG_SCHEDULE_REJS') ?></th>
                            <th><?php echo  __('dictionary.MSG_MSG_SCHEDULE_MARSHRUT')?></th>
                            <th><?php echo  __('dictionary.MSG_MSG_SCHEDULE_VARTISTI') ?></th>
                            <th><?php echo  __('dictionary.MSG_MSG_SCHEDULE_POSILANNYA_NA_BRONYUVANNYA') ?></th>
                        </tr>
                        </thead>
                ...
                WHERE station.station = 1 AND station.section_id = '" . $route['arrival'] . "' AND station.id IN (SELECT stop_id FROM `" . DB_PREFIX . "_tours_stops`WHERE tour_id = '" . $route['id'] . "')");
                                ...
                                <tr class="shedule_tr">
                                    <?php if ($k == 0) { ?>
                                        <td class="shedule_td manrope" rowspan="<?php echo  count($routes) ?>">
                                            <?php echo  $route['departure_country'] ?>
                                        </td>
                                    <?php } ?>
                                    <td class="shedule_td manrope"><?php echo  $route['departure_city'] . ' - ' . $route['arrival_city'] ?></td>
                                    <td class="shedule_td">
                                        <button class="schedule_details_btn"
                                                onclick="toggleRouteDetailsSchedule('<?php echo  $route['id'] ?>', '<?php echo  $departureDetails['id'] ?>', '<?php echo  $arrivalDetails['id'] ?>')">
                                            <?php echo  __('dictionary.MSG_MSG_SCHEDULE_GRAFIK_I_RASPISANIE_REJSA') ?>
                                        </button>
                                    </td>
                                    <td class="shedule_td h4_title">
                                        ...
                                        <button class="schedule_details_btn"
                                                onclick="toggleRoutePricesSchedule('<?php echo  $route['id'] ?>', '<?php echo  $departureDetails['id'] ?>', '<?php echo  $arrivalDetails['id'] ?>', '<?php echo  $nearestDepartureDate ?>')">
                                            <?php echo  __('dictionary.MSG_MSG_SCHEDULE_PRICE_TABLE') ?>
                                        </button>
                                    </td>
                                    <td class="shedule_td">
                                        <button class="buy_btn h5_title"
                                                onclick="buyTicketFromSchedule(this, '<?php echo  $route['id'] ?>', '<?php echo  $departureDetails['id'] ?>', '<?php echo  $arrivalDetails['id'] ?>', '<?php echo  $nearestDepartureDate ?>')">
                                            <?php echo  __('dictionary.MSG_MSG_SCHEDULE_KUPITI_KVITOK') ?>
                                        </button>
                                    </td>
                                </tr>
                                <?php
        ...
        <div class="routes_block">
            <div class="container">
                <div class="routes_title h2_title">
                    <?php echo  __('dictionary.MSG__NASHI_NAPRAVLENNYA') ?>
                </div>
                <div class="routes_subtitle par">
                    <?php echo  __('dictionary.MSG__BEZLICH_VARIANTIV_AVTOBUSNIH_POZDOK_DLYA_VASHIH_PODOROZHEJ_U_BUDI') ?>
                </div>
                <div class="routes_lists_wrapper">
                    <div class="route_list_block">
                        <div class="route_list_title h3_title"><?php echo  __('dictionary.MSG_ALL_KRANI') ?></div>
                    ...
                    <div class="route_list_block">
                        <div class="route_list_title h3_title"><?php echo  $GLOBALS['dictionary']['MSG_ALL_ROZKLAD'] ?></div>
                        <div class="route_list">
    ...
    function buyTicketFromSchedule(item, id, departure, arrival, date) {
        ...
        $.ajax({
            ...
            success: function (response) {
                removeLoader();
                if ($.trim(response) == 'ok') {
                    location.href = '<?php echo $Router->writelink(85)?>';
