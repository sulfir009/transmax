...
if ($Admin->CheckPermission($_params['access'])) {
    ...
    if ($getRouteDetails){
        ...
        $getPassengers = $Db->getAll("SELECT
                            ...
                            o.client_phone,
                            o.uniqid,
                            op.name AS client_name,
                            op.second_name,
                            op.patronymic,
                            o.payment_status,
                            o.client_name AS order_client_name,
                            o.client_surname AS order_client_surname,
                            o.client_email,
                            o.client_phone,
                            ...
                            LEFT JOIN `" .  DB_PREFIX . "_orders_passangers`op ON op.order_id = o.uniqid
        ...
        foreach ($getPassengers AS $k=>$passenger){
            ...
            if($passenger['passagers'] ==='1') {
                $clientname = $passenger['order_client_name']. ' ' .$passenger['order_client_surname'];
            } else {
                $clientname = $passenger['client_name'].' '.$passenger['second_name'].' '.$passenger['patronymic'];
            }
