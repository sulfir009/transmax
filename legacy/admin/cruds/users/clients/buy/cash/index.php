...
if ($Admin->CheckPermission($_params['access'])) {
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-valign-middle">
                                    ...
                                    // Получаем заказы с оплатой наличными через репозиторий
                                    $getOrders = $clientRepository->getClientsWithCashPayment($Admin->lang, $offset, $_params['num_page']);
                                    
                                    // Получаем общее количество для пагинации
                                    $total = $clientRepository->countOrdersByPaymentType(1, false);
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td><?=$order->phone?></td>
                                            <td><?=$order->email?></td>
                                            <td><span class="badge badge-warning">Наличными</span></td>
                                            <td><?=date('d.m.Y H:i', strtotime($order->date))?></td>
                                            <td><?=$order->departure_city . ' - ' . $order->arrival_city?></td>
                                            <td align="center">
                                                <div class="btn-group">
                                                    <button type="button" class="btn btn-sm btn-info" onclick="sendBooking(<?=$order->id?>)">
                                                        <i class="fas fa-envelope"></i> Отправить бронь
                                                    </button>
    ...
    <script>
    function sendBooking(orderId) {
        if (!confirm('Отправить бронь билета клиенту?')) {
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    alert('Ошибка при отправке брони: ' + (response.message || 'Неизвестная ошибка'));
                }
            },
            error: function() {
                removeLoader();
