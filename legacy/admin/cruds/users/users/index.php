                ...
                <div class="container-fluid">
                    <div class="table-responsive table-striped table-valign-middle">
                        <table class="table m-0">
                            ...
                            <tbody>
                            <? $getElems = mysqli_query($db, "SELECT id,name,image FROM `" .  DB_PREFIX . "_users`WHERE `usergroup` >= " . $Admin->permissions . " ORDER BY id ASC ");
                            while ($Elem = mysqli_fetch_assoc($getElems)) {
                                ?>
                                <tr>
                                    <td><?= $Elem['id'] ?></td>
                                    <td><img src="<?= asset('images/legacy/upload/main/' . $Elem['image']); ?>" alt="" style="max-width: 100px"></td>
                                    <td><?= $Elem['name'] ?></td>
