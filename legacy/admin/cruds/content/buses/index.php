                    ...
                    <div class="table-responsive table-striped table-valign-middle">
                        <table class="table m-0">
                            ...
                            while ($Elem = mysqli_fetch_assoc($getTableElems)) {
                                $imagesResult = mysqli_query($db, "SELECT id, bus_img FROM `" .  DB_PREFIX . "_buses_images`WHERE bus_id='" . $Elem['id'] . "'");
                                $image = mysqli_fetch_array($imagesResult);
                                ?>
                                <tr <?if ($Elem['active'] == '0'){?>
                                  class="disabled"
                                <?}?>>
                                    ...
                                    <td><?php
                                        ...
                                        if ($image) { ?>
                                            <img style="max-width: 300px;" src="<?= asset('images/legacy/upload/buses/' . $image['bus_img']); ?>">
                                        <?php } else { ?>
