...
When multiple canvases and SVGs have to be overlapped into a single images
...
problems being converted to an images and makes sure the promises system,
...
failure in generating the temporary images. Temporary images are required
mostly on extracting content from SVGs, but this is also where the x/y
offsets are extracted for each images which will be added. For SVGs in
...
(function($) {
    ...
    function getGenerateTempImg(tempImg, canvasOrSvgSource) {
        ...
        return function doGenerateTempImg(successCallbackFunc, failureCallbackFunc) {
            ...
            tempImg.onabort = function(evt) {
                ...
                console.log('Can\'t generate temp images from ' + tempImg.sourceDescription + '. It is possible that it is missing some properties or its content is not supported by this browser. Source component:', tempImg.sourceComponent);
    ...
    function copySVGToImgMostBrowsers(svg, img) {
        ...
        var blob = new Blob([source], {type: "images/svg+xml;charset=utf-8"}),
    ...
    function copySVGToImgSafari(svg, img) {
        ...
        data = "data:images/svg+xml;base64," + btoa(utf8BinaryString);
        img.src = data;
    }
