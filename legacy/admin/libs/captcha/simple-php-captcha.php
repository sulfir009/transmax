...
function simple_php_captcha($config = array()) {
    ...
    if ( strpos($_SERVER['SCRIPT_FILENAME'], str_replace('public', 'legacy', $_SERVER['DOCUMENT_ROOT'])) ) {
        $image_src = substr(__FILE__, strlen( realpath(str_replace('public', 'legacy', $_SERVER['DOCUMENT_ROOT'])) )) . '?_CAPTCHA&amp;t=' . urlencode(microtime());
