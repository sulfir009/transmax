                ...
                if (count($pastRidesArray) > 0){ ?>
                    <div class="private_tickets_wrapper">
                        <div class="flex-row gap-30">
                            <?foreach ($pastRidesArray AS $k=>$history){
                                ...
                                $rideTime = calculateTotalTravelTime($getTicketStops,$history['from_stop'],$history['to_stop'], $history['arrival_day'] ?? 0);?>
                                <div class="col-xxl-6">
                                    <div class="ticket_card flex-row private_ticket shadow_block">
                                        <div class="col-md-12">
                                            <div class="ticket_info">
                                                <div class="ticket_info_header flex_ac">
                                                    ...
                                                    <?=date('d',strtotime($history['tour_date'])).' '.$month['title'].' '.date('Y',strtotime($history['tour_date']))?>
                                                </span>
                                                    </div>
                                                    <div class="ride_description_wrapper flex_ac">
                                                        <div class="ride_description par">
                                                            ...
                                                            <span><?=$history['departure_city'].' - '.$history['arrival_city']?></span>
                                                        </div>
                                                        <div class="ride_description par">
                                                            <span><?= $GLOBALS['dictionary']['MSG_MSG_TICKETS_AVTOBUS'] ?></span>
                                                            <span><?=$history['bus_title']?></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="ticket_ride_info_block flex-row gap-30">
                                                    <div class="col-lg-4 col-sm-6 col-xs-12">
                                                        <div class="ticket_ride_departure ticket_ride_info">
                                                            <div class="ticket_ride_time flex_ac">
                                                                ...
                                                                <span class="btn_txt"><?=date('H:i',strtotime($history['departure_time']))?></span>
                                                            </div>
                                                            <div class="ticket_ride_city btn_txt">
                                                                <?=$history['departure_city']?>
                                                            </div>
                                                            <div class="ticket_ride_checkpoint manrope">
                                                                <?=$history['departure_station']?>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-lg-4 col-sm-6 col-xs-12">
                                                        <div class="ticket_ride_arrival ticket_ride_info">
                                                            <div class="ticket_ride_time flex_ac">
                                                                <img src="<?= asset('images/legacy/common/clock.svg'); ?>" alt="clock">
                                                                <span class="btn_txt"><?=date('H:i',strtotime($history['arrival_time']))?></span>
                                                            </div>
                                                            <div class="ticket_ride_city btn_txt">
                                                                <?=$history['arrival_city']?>
                                                            </div>
                                                            <div class="ticket_ride_checkpoint">
                                                                <?=$history['arrival_station']?>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <button class="ticket_details_btn shedule_link flex_ac hidden-md hidden-sm hidden-xs" onclick="toggleRouteDetailsHistory('<?=$history['tour_id']?>','<?= $history['from_stop'] ?>','<?= $history['to_stop'] ?>')">
                                        ...
                                        <div class="col-lg-12 hidden-md hidden-sm hidden-xs">
                                            <div class="ticket_totals">
                                                <div class="ticket_price private_ticket_price"><?=$history['price']?> ₴</div>
                                                <button class="ticket_buy_btn h5_title blue_btn flex_ac" onclick="toggleRouteDetailsHistory('<?=$history['tour_id']?>','<?= $history['from_stop'] ?>','<?= $history['to_stop'] ?>')"><?= $GLOBALS['dictionary']['MSG_MSG_TICKETS_DETALINISHE'] ?></button>
                                            </div>
                                        </div>
                                            ...
                                            <div class="ride_total_time">
                                                ...
                                                <div class="mobile_ticket_ride_total_time_wrapper flex_ac">
                                                    <div class="ticket_ride_total_time_info flex_ac">
                                                        <img src="<?= asset('images/legacy/common/info.svg'); ?>" alt="info">
                                                        <div class="ticket_ride_total_time par">
                                                            <?=$history['hours'].' '.$GLOBALS['dictionary']['MSG_MSG_TICKETS_GOD'].' '.$history['minutes'].' '.$GLOBALS['dictionary']['MSG_MSG_TICKETS_HV_V_DOROZI'] ?>
                                                        </div>
                                                    </div>
                                                    <?if ($international){?>
                                                        <div class="ticket_ride_status par">
                                        ...
                                        <div class="hidden-xxl hidden-xl hidden-lg col-xs-12">
                                            <div class="mobile_ticket_totals flex_ac">
                                                <div class="mobile_ticket_details flex_ac">
                                                    <div class="ticket_price"><?=$history['price']?> ₴</div>
                                                </div>
                                                <button class="ticket_buy_btn h5_title blue_btn flex_ac" onclick="toggleRouteDetailsHistory('<?=$history['tour_id']?>','<?= $history['from_stop'] ?>','<?= $history['to_stop'] ?>')"><?= $GLOBALS['dictionary']['MSG_MSG_TICKETS_DETALINISHE'] ?></button>
                                            </div>
                                        </div>
                ...
                <div class="container">
                    <div class="private_empty_block">
                        <div class="private_empry_block_title h2_title">
                            <?=$GLOBALS['dictionary']['MSG_MSG_HISTORY_U_VAS_POKI_NEMA_ISTORI_POZDOK']?>
                        </div>
                        <div class="private_empry_block_subtitle">
                            <?=$GLOBALS['dictionary']['MSG_MSG_HISTORY_SCHOB_ZNAJTI_KVITKI_NA_AVTOBUS']?>
                        </div>
                    </div>
                </div>
