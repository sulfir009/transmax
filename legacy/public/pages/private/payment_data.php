...
mt_cabinet_auth_log($User, $Router, 'payment_data');
...
if ((int)$User->id <= 0) {
    \Illuminate\Support\Facades\Log::channel('cabinet')->warning('[CABINET AUTH] Missing client_id on payment_data page', [
...
<div class="wrapper">
    ...
    <div class="content">
        <div class="private_links_wrapper">
            <div class="tabs_links_container hidden-xs">
                <div class="private_links">
                    ...
                    <div class="private_link_wrapper">
                        <a href="<?=$privateMenuLinks[82]['url']?>" class="private_tab h4_title active">
        ...
        <div class="page_content_wrapper">
            <div class="container">
                <?$clientCard = $Db->getOne("SELECT * FROM `" .  DB_PREFIX . "_clients_cards`WHERE client_id = '".$User->id."' ");
                if (!$clientCard){?>
                    <div class="private_empty_block">
                        <div class="private_empty_block_title h2_title">
                            <?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_U_VAS_POKI_NE_PIDDNANI_SPOSOBI_OPLATI']?>
                        </div>
                        <div class="private_empty_block_subtitle">
                            <?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_VVEDITI_DANI_KARTKI_SCHOB_VAM_BULO_ZRUCHNISHE_KUPUVATI_BILETI']?>
                        </div>
                    </div>
                <?}else{?>
                    <div class="flex-row gap-x-30 gap-y-60">
                        <div class="col-xxl-7">
                            <div class="shadow_block private_payment_data_block">
                                <div class="private_payment_data_header h2_title">
                                    <?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_DANI_KARTKI']?>
                                </div>
                                    ...
                                    <div class="col-xxl-6">
                                        ...
                                        <div class="payment_form_fields">
                                            <div class="flex-row gap-30">
                                                <div class="col-md-12">
                                                    <input type="text" class="c_input" placeholder="<?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_XXXX_XXXX_XXXX_XXXX']?>" id="card_number"
                                                        <?if (trim($clientCard['card_number']) != ''){?>
                                                            value="<?=$clientCard['card_number']?>"
                                                        <?}?>>
                                                </div>
                                                <div class="col-xs-4">
                                                    <input type="text" class="c_input card_valid_date" placeholder="<?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_12_/_25']?>" id="card_valid_date"
                                                        <?if (trim($clientCard['valid_date']) != ''){?>
                                                            value="<?=$clientCard['valid_date']?>"
                                                        <?}?>
                                                    >
                                                </div>
                                                <div class="col-xs-8">
                                                    <div class="row">
                                                        <input type="password" class="c_input cvv_input" placeholder="***" id="card_cvv"
                                                            <?if ((int)$clientCard['cvv'] > 0){?>
                                                                value="<?=$clientCard['cvv']?>"
                                                            <?}?>
                                                        >
                                                ...
                                                <div class="col-md-12">
                                                    <input type="text" class="c_input" placeholder="<?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_IMYA_VLASNIKA_KARTKI']?>" id="cardholder_name"
                                                        <?if (trim($clientCard['cardholder_name']) != ''){?>
                                                            value="<?=$clientCard['cardholder_name']?>"
                                                        <?}?>
                                                    >
                                                </div>
                                            </div>
                                    ...
                                    <div class="col-xxl-6">
                                        <div class="payment_form_right">
                                            <div class="payment_form_image">
                                                <img src="<?= asset('images/legacy/common/credit_card.png'); ?>" alt="credit card" class="fit_img">
                                            </div>
                                            <button class="blue_btn h4_title edit_private_payment_btn" onclick="saveClientPaymentData()">
                                                <img src="<?= asset('images/legacy/common/pencil.svg'); ?>" alt="pencil">
                                                <?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_ZMINITI_DANI']?>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-xxl-5">
                            <div class="private_payments_clarification_block">
                                <img src="<?= asset('images/legacy/cards.png'); ?>" alt="cards" class="fit_img">
                                <div class="private_payments_clarification_txt h5_title">
                                    <?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_VSYA_PLATIZHNA_INFORMACIYA']?>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        $.ajax({
           type:'post',
            ...
            success:function(response){
                removeLoader();
                if ($.trim(response) == 'ok'){
                    out('<?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_DANNYE_USPESHNO_SOHRANENY']?>');
                }else if($.trim(response) == 'err'){
                    out('<?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_NE_UDALOSI_SOHRANITI_DANNYE']?>','<?=$GLOBALS['dictionary']['MSG_MSG_PRIVATE_PAYMENT_DATA_POPROBUJTE_POZZHE']?>');
                }
            }
        })
    }
